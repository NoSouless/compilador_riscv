...
class RAM{
public:
    // Memory map (addresses inclusive ranges)
...
private:
    std::vector<uint8_t> mem; // byte-addressable memory backing (BACKING_SIZE bytes)
public:
    RAM() : mem(BACKING_SIZE, 0u) {
    }

    // Write a 32-bit little-endian word at byte address
    ...
    // Read a 32-bit little-endian word at byte address
